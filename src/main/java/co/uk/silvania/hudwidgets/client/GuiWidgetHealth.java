...
public class GuiWidgetHealth extends GuiWidgetBase {
	...
	public void onRenderGui(RenderGameOverlayEvent.Pre event) {
		...
		if (Minecraft.getMinecraft().player.capabilities.isCreativeMode && !HUDWidgetsConfig.renderHealthCreative) {
			enabled = false;
		}
		if (debugMode) {
			enabled = false;
		...
		if (enabled) {
			...
			double health = Math.round(Minecraft.getMinecraft().player.getHealth());
			double maxHealth = Math.round(Minecraft.getMinecraft().player.getMaxHealth());
			double healthAmount = Math.round((200 / maxHealth) * health);
			int armourAmount = (158 / 75) * Minecraft.getMinecraft().player.getTotalArmorValue();
			int oxygenAmount = Math.round(Minecraft.getMinecraft().player.getAir() / 2);
			
			int healthLast = MathHelper.ceiling_float_int(Minecraft.getMinecraft().player.prevHealth);
	    	
	    	boolean highlight = Minecraft.getMinecraft().player.hurtResistantTime / 3 % 2 == 1;
            if (Minecraft.getMinecraft().player.hurtResistantTime < 10) { highlight = false; }
			...
			boolean poisoned = Minecraft.getMinecraft().player.isPotionActive(Potion.poison);
			boolean withered = Minecraft.getMinecraft().player.isPotionActive(Potion.wither);
			...
			if (HUDWidgetsConfig.armourBarStyle == 2) {
				if (HUDWidgetsConfig.alwaysRenderArmour || Minecraft.getMinecraft().player.getTotalArmorValue() > 0) {
