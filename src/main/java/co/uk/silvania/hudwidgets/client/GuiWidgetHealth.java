...
public class GuiWidgetHealth extends GuiWidgetBase {
	...
	public void onRenderGui(RenderGameOverlayEvent.Pre event) {
		...
		if (mc.getMinecraft().player.capabilities.isCreativeMode && !HUDWidgetsConfig.renderHealthCreative) {
			enabled = false;
		}
		if (debugMode) {
			enabled = false;
		...
		if (enabled) {
			...
			double health = Math.round(mc.getMinecraft().player.getHealth());
			double maxHealth = Math.round(mc.getMinecraft().player.getMaxHealth());
			double healthAmount = Math.round((200 / maxHealth) * health);
			int armourAmount = (158 / 75) * mc.getMinecraft().player.getTotalArmorValue();
			int oxygenAmount = Math.round(mc.getMinecraft().player.getAir() / 2);
			
			int healthLast = MathHelper.ceiling_float_int(mc.getMinecraft().player.prevHealth);
	    	
	    	boolean highlight = mc.getMinecraft().player.hurtResistantTime / 3 % 2 == 1;
            if (mc.getMinecraft().player.hurtResistantTime < 10) { highlight = false; }
			...
			boolean poisoned = mc.getMinecraft().player.isPotionActive(Potion.poison);
			boolean withered = mc.getMinecraft().player.isPotionActive(Potion.wither);
			...
			if (HUDWidgetsConfig.armourBarStyle == 2) {
				if (HUDWidgetsConfig.alwaysRenderArmour || mc.getMinecraft().player.getTotalArmorValue() > 0) {
			...
			if (HUDWidgetsConfig.oxygenBarStyle == 2) {
				if (mc.getMinecraft().player.getAir() < 300) {
